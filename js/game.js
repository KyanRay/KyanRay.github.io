...
class Game {
  ...
  drawText (obj) {
    ...
    // 绘制分数
    this.context.fillText(obj.text + obj.allScore + '分钟', obj.x, obj.y)
    // 绘制关卡
  ...
  gameOver () {
    ...
    this.drawBg()
    // 绘制提示文字
    this.context.font = '96px Microsoft YaHei'
    this.context.fillStyle = '#fff'
    this.context.fillText('安心加班吧', 404, 226)
  }
  // 游戏晋级
  goodGame () {
    // 清除定时器
    ...
    this.context.font = '96px Microsoft YaHei'
    this.context.fillStyle = '#fff'
    this.context.fillText('直接下班！', 308, 226)
  }
  // 游戏通关
  finalGame () {
    // 清除定时器
    ...
    this.context.font = '96px Microsoft YaHei'
    this.context.fillStyle = '#fff'
    this.context.fillText('以后再也不开会！', 308, 226)
  }
  // 注册事件
  ...
  showPopupText(text, x, y) {
    console.log("showPopupText called"); 
    this.context.font = '24px Microsoft YaHei';
    this.context.fillStyle = 'white'; // 设置文字颜色为红色，您可以根据需要更改颜色
    this.context.fillText(text, x, y);
    // 可以添加逻辑以控制弹出文字的展示时间和效果
  }
  checkBallBlock (g, paddle, ball, blockList, score) {
    ...
    if (p.collide(b)) {
      ...
      g.showPopupText("-100 min", b.x, b.y);
      // 设置X轴速度
